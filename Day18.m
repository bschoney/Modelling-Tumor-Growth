...
nrow = round(sqrt(length(cells(1,1,:,5)))); % number of subplots to be created in the y direction
...
mainPlot.Title.String = 'Time = 18.0 days. Brian MRI slices alon Z-direction, Rat W09. No raditation treatment'; % set the title of the figure
...
for irow = nrow:-1:1
    for icol = 1:ncol
        ...
        subPlot = axes( 'position', [ ... set the position of the axes for each subplot
                ...
                imagesc(cells_5(:,:,sliceNumber),[0 (max(max(max(cells_5))))]);
