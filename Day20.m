...
nrow = round(sqrt(length(cells(1,1,:,6)))); % number of subplots to be created in the y direction
...
colorLimits = [0,max(max(max(cells(:,:,:))))];                    % the color bar limits, the cellsset contains numbers between 0 and some large number.
...
mainPlot.Title.String = 'Time = 20.0 days. Brian MRI slices alon Z-direction, Rat W09. No raditation treatment'; % set the title of the figure
...
for irow = nrow:-1:1
    for icol = 1:ncol
        ...
        subPlot = axes( 'position', [ ... set the position of the axes for each subplot
                ...
                imagesc(cells_6(:,:,sliceNumber),[0 (max(max(max(cells_6))))]);
